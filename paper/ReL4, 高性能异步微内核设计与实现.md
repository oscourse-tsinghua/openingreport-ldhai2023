...
从图中可以看出，在原始的通知机制中，通信的数据流向是一个冗余的连通图，只要拥有合适的capability，所有的Sender TCB可以通过任意一个notificaiton对象向任意一个Receiver TCB发送通知，反之也成立。而在被用户态中断改造的U-notification中，由于接收端对TCB的独占性，导致了Receiver TCB对其他notification object的连接边失效，但由于原始连通图的冗余性，新的数据流通图仍然是连通图，保持了通信权限控制的完备性。
