...
本文聚焦微内核的异步通信机制，利用用户态中断技术改造seL4的通知机制，使得信号无需通过内核转发，减少上下切换的开销。同时利用无需内核转发的通知机制，设计和实现无需频繁陷入内核的异步系统调用和异步IPC框架，在提升用户态并发度的同时，减少用户态内核态和用户态切换的次数，提升系统的整体性能。
...
本选题旨在调研现代微内核的IPC发展趋势及性能瓶颈，减少微内核的IPC开销，从而提升微内核系统的资源利用率、IPC时延和吞吐率。
...
<table>
    ...
    <tr>
        <td rowspan="3">减少内核路径</td><td>快速路径 </td><td rowspan="3">[2, 8, 18, 19]</td> <td rowspan="3"> 上下文切换开销已经成为性能瓶颈 </td>
    </tr> 
    <tr>
        <td>快速路径</td>
    </tr>
    ...
    <tr>
        <td rowspan="5">减少上下文切换开销 </td> <td> 消息寄存器 </td>  <td rowspan="2"> [2, 8, 18, 19] </td> <td rowspan="3"> 无法从根本上消除切换开销 </td>
    </tr>
    <tr>
        <td>组合系统调用</td>
    </tr>
